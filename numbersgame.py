...
def write_to_csv(username, attempts, level):
    with open("numbersgame.csv", mode="a", newline="") as file:
        ...
        writer.writerow(
            [username, attempts, level, datetime.now().strftime("%Y-%m-%d %H:%M:%S")]
        )
...
def level_3(username):
    ...
    while True:
        ...
        for i in range(3):
            if (
                program_number_numbers[i] is not None
                and program_number_numbers[i] in user_guess_numbers
            ):
                wrong_spot += 1
                user_guess_numbers[
                    user_guess_numbers.index(program_number_numbers[i])
                ] = None
